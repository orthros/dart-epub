...
class RootFilePathReader {
  static Future<String> GetRootFilePathAsync(Archive epubArchive) async  {
    ...
    ArchiveFile containerFileEntry = epubArchive.files
                                                .firstWhere((ArchiveFile file) => file.name == EPUB_CONTAINER_FILE_PATH, orElse: () => null);
    if (containerFileEntry == null)
        throw new Exception("EPUB parsing error: ${EPUB_CONTAINER_FILE_PATH} file not found in archive.");
    xml.XmlDocument containerDocument = xml.parse(UTF8.decode(containerFileEntry.content));      
    xml.XmlElement packageElement = containerDocument.findAllElements("container", namespace: "urn:oasis:names:tc:opendocument:xmlns:container")
